...
extension Publishers {
    ...
    public struct TryFilter<Upstream> : Publisher where Upstream : Publisher {
        ...
        ///                   once attached it can begin to receive values.
        public func receive<S>(subscriber: S)
            where S : Subscriber, Upstream.Output == S.Input, S.Failure == Publishers.TryFilter<Upstream>.Failure {
            let filter = _Filter<Upstream, S>(downstream: subscriber, isIncluded: isIncluded)
            upstream.receive(subscriber: filter)
        }
    }
...
//    public func filter(_ isIncluded: @escaping (Publishers.Filter<Upstream>.Output) -> Bool)
//        -> Publishers.Filter<Upstream> {
//        return Publishers.Filter(upstream: upstream) { isIncluded($0) && self.isIncluded($0) }
//    }
//
//    public func tryFilter(_ isIncluded: @escaping (Publishers.Filter<Upstream>.Output) throws -> Bool)
...
//
//    public func filter(_ isIncluded: @escaping (Publishers.TryFilter<Upstream>.Output) -> Bool)
//        -> Publishers.TryFilter<Upstream> {
//        return Publishers.TryFilter(upstream: upstream)  { try isIncluded($0) && self.isIncluded($0) }
//    }
//
//    public func tryFilter(_ isIncluded: @escaping (Publishers.TryFilter<Upstream>.Output) throws -> Bool)
//        -> Publishers.TryFilter<Upstream> {
//        return Publishers.TryFilter(upstream: upstream) { try isIncluded($0) && self.isIncluded($0) }
//    }
//}
